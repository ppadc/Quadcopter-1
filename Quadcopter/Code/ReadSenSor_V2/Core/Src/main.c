...
Quaternion_t Q_est_LP = {1,0,0,0};
Quaternion_t Duty = {955, 955, 955, 955};
Quaternion_t Duty_nen = {45, 45, 45, 45};
Quaternion_t LimDuty = {1400, 1400, 1400, 1400};
PID_t PID_roll = {
	.Kp = 0,
	.Ki = 0,
	.Kd = 0,
...
// ESC 
void setupESC();
void ESC_Control(Quaternion_t duty,Quaternion_t duty_nen);
// PID
float PID_Control(float Sp, float Fb, PID_t *pid);
void Combining_Angle(Quaternion_t duty, float roll, float pitch, float yaw);
void Combining_Angle_height(Quaternion_t *duty, float roll, float pitch, float yaw, float height);
...
void ESC_Control(Quaternion_t duty,Quaternion_t duty_nen)
{
	__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, duty.q1 + duty_nen.q1); // 1000 to 2000
	__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_2, duty.q2 + duty_nen.q2); // 1000 to 2000
	__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_3, duty.q3 + duty_nen.q3); // 1000 to 2000
	__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_4, duty.q4 + duty_nen.q4); // 1000 to 2000
}
float PID_Control(float Sp, float Fb, PID_t *pid)
{
	float out;
	...
	{
		pid->e_i += pid->e * SIMPLE_TIME;
		if (pid->e_i > pid->Upper_limit)
			pid->e_i = pid->Upper_limit;
		if (pid->e_i < pid->Lower_limit)
			pid->e_i = pid->Lower_limit;
	}
	else
		pid->e_i = 0;
...
void Combining_Angle_height(Quaternion_t *duty, float roll, float pitch, float yaw, float height)
{
	duty->q1 = 955 +  pitch + yaw + height;
	duty->q2 = 955 +  roll  - yaw + height;
	duty->q3 = 955 + -pitch + yaw + height;
	duty->q4 = 955 + -roll  - yaw + height;
	// limit duty
	if(duty->q1 > LimDuty.q1) duty->q1 = LimDuty.q1;
	if(duty->q2 > LimDuty.q2) duty->q2 = LimDuty.q2;
	if(duty->q3 > LimDuty.q3) duty->q3 = LimDuty.q3;
	if(duty->q4 > LimDuty.q4) duty->q4 = LimDuty.q4;
	if(duty->q1 < 955) 			duty->q1 = 955;
	if(duty->q2 < 955) 			duty->q2 = 955;
	if(duty->q3 < 955) 			duty->q3 = 955;
	if(duty->q4 < 955) 			duty->q4 = 955;
}
void Lowpass(float in,LPF_t *filter, float iCutOffFrequency)
{	
	filter->input = in;
	filter->ePow = (1-exp(-SIMPLE_TIME * 2 * PI * iCutOffFrequency));
	filter->output += (filter->input - filter->output) * filter->ePow;
}
void plot(Euler_t angle)
{
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	if (htim->Instance == htim2.Instance)
	{
		...
		MPU6050_Read_All(&hi2c1, &MPU6050); // roll pitch yaw
		MS5611_calculate(&MS5611);
		//Filter
		Lowpass(MS5611.P,&LP_height,0.3);
		MPU6050_LP = MPU6050;
		Lowpass(MPU6050.Ax,&LP_Ax,1);MPU6050_LP.Ax = LP_Ax.output;
		Lowpass(MPU6050.Ay,&LP_Ay,1);MPU6050_LP.Ay = LP_Ay.output;
		Lowpass(MPU6050.Az,&LP_Az,1);MPU6050_LP.Az = LP_Az.output;
		Lowpass(MPU6050.Gx,&LP_Gx,1);MPU6050_LP.Gx = LP_Gx.output;
		Lowpass(MPU6050.Gy,&LP_Gy,1);MPU6050_LP.Gy = LP_Gy.output;
		Lowpass(MPU6050.Gz,&LP_Gz,1);MPU6050_LP.Gz = LP_Gz.output;
		
		Madgwick_update(&MPU6050, &Madgwick, &Q_est);
		Madgwick_update(&MPU6050_LP, &Madgwick_LP, &Q_est_LP);
		
		height = getAltitude(LP_height.output,101325); // Z
		//Control
		...
		if(enable)
		{
			Duty_nen.q1 = 45;Duty_nen.q2 = 0;Duty_nen.q3 = 45;Duty_nen.q4 = 0;
			Combining_Angle_height(&Duty,roll,pitch,yaw,Z);//1000 to 2000
			ESC_Control(Duty,Duty_nen);
		}
		else
		{
			Duty_nen.q1 = 0;Duty_nen.q2 = 0;Duty_nen.q3 = 0;Duty_nen.q4 = 0;
			Duty.q1 = 955;Duty.q2 = 955;Duty.q3 = 955;Duty.q4 = 955;
			ESC_Control(Duty,Duty_nen);
		}
	}
}
