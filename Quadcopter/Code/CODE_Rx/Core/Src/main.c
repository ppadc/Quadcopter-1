...
void tackchuoi(uint8_t* str)
{
char *token; // Con trỏ để lưu từng token
float arr[4]; // Mảng để lưu 4 số float
int i = 0; // Biến đếm

    // Sử dụng hàm strtok để tách chuỗi theo dấu ;
    token = strtok(str, ";");
    while (token != NULL)
    {
        // Chuyển token từ chuỗi sang số float và lưu vào mảng
        arr[i] = atof(token);
        i++;
        // Tiếp tục tách chuỗi
        token = strtok(NULL, ";");
